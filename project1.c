...
int main() {
	...
	int dlzka = strlen(vojaci); //zistenie dlžky pola, obe polia sú vždy rovnako dlhé
	...
	for( int index1=0; index1 <dlzka-1; index1++ )
		...
		for( int index2= index1 +1; index2<dlzka; index2++ ) {
			...
			if ( pocet_mesiacov[index1] > pocet_mesiacov[index2] ) { //zistenie èi má nasta výmena vojakov

				if ( vojaci[index2] == vojaci[index1] ) { //hodnoty sa vymeniaa iba v prípade že akú na oboch miestach rovnaký vojaci
					vymena = pocet_mesiacov[index1]; //uloženie do premennej
	...
	int index_vypis = 0;
	while (index_vypis != dlzka) { //vypísanie výstupu na obrazovku cez jeden index z oboch arrays kedže údaje z rovnakého indexu vždy patria jednému vojakovi
